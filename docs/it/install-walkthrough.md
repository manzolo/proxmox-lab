...
> **Percorso rapido**: `make wizard WIZARD_LANG=it` esegue tutti i passi di questa guida
> in modo guidato e interattivo, con pausa di conferma ad ogni fase.
...
| `USE_TAP_NETWORK` | `1` per cluster | vedere sotto |
...
Crea il bridge, i TAP, assegna `BRIDGE_ADDRESS` al bridge e aggiunge automaticamente le
regole NAT/MASQUERADE iptables in modo che i nodi possano raggiungere internet durante l'install.

Per verificare:

```bash
ip addr show manzolo-br0   # deve mostrare 192.168.100.1/24
```

Le regole NAT vengono rimosse automaticamente da `sudo make network-down`. Per gestirle
separatamente:

```bash
sudo make network-nat-up    # aggiunge solo le regole NAT
sudo make network-nat-down  # rimuove solo le regole NAT
```
...
1. **Scambio chiavi SSH**: genera una chiave ed25519 su pve02 e pve03 (se assente), aggiunge le chiavi pubbliche agli `authorized_keys` di pve01 e pre-accetta il fingerprint di pve01 — necessario per `pvecm add -use_ssh 1`
...
```bash
make stop
sudo make network-down   # rimuove TAP e regole NAT
make clean-all
```
...
visibile nella UI sotto Datacenter → Storage.
